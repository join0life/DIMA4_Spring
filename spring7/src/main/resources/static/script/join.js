 ...
 */
/**
 * 전역변수 : 아이디, 비번이 올바를 경우에만 가입 가능
 */
let idCheck = false; // 가입 불가능한 상태
let pwdCheck = false;
...
$(function () {
	...
	$('#userPwd').on('focus', function() {
		$('#userPwdCheck').val(''); // userPwd 부분에 포커스를 맞추면 userPwdCheck 값이 사라짐
	});
	$('#submitBtn').on('click', join);
});

// 비밀번호, 비밀번호 확인, 이름이 입력되었는지 확인 ==> idCheck, pwdCheck 값이 true일 경우에만 가입
function join () {
